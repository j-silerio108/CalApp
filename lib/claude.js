 ...
 * @returns {Promise<{course_name: string|null, items: Array<object>}>}
 */
export async function extractDueDates(syllabusText, referenceYear = new Date().getFullYear()) {
  const response = await client.messages.create({
    ...
    system:
      `You read college course syllabi and extract every date something is due. ` +
      ...
      `reasonable guess rather than skipping the item. Only call the tool once, with every item included.`,
    messages: [
      {
        role: "user",
